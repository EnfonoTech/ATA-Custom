...
def ask(question):
	...
	if not q:
		return {
			"answer": "Please ask me a question about your projects or files.",
			"data": [],
			"type": "text",
		}
	...
	if any(k in ql for k in ["file", "upload", "document"]) and any(
		k in ql for k in ["recent", "last", "week", "today", "this month"]
	):
		days = 7
		if "today" in ql:
			days = 1
	...
	if allowed:
		...
		if terms and any(k in ql for k in ["find", "search", "show", "list", "which", "what", "projects"]):
			placeholders = ",".join(["%s"] * len(allowed))
			kw_clauses = " OR ".join(
				"(LOWER(project_name) LIKE %s OR LOWER(IFNULL(portal_project_code,'')) LIKE %s)"
				for _ in terms
			)
			...
			if rows:
				names = [
					f"{r.get('portal_project_code') or ''} — {r.project_name} ({r.status})".strip("— ")
					for r in rows
				]
				return {
					"type": "list",
